...
class UniversalClient(LLMBackend):
    ...
    def _get_payload(self, messages: list[LLMMessage], stream: bool, temperature: float = 0.7, top_p: float = 1.0, response_format: str | None = None, stop_sequences: list[str] | None = None, max_tokens: int | None = None, response_schema: dict | None = None) -> dict:
        ...
        if response_format == "json":
            # `json_object` guarantees syntactically valid JSON and is accepted by
            # every OpenAI-compatible provider we ship (Gemini/Claude/Venice/
            # Fireworks/OpenAI/OpenRouter). The stricter `json_schema` type is NOT
            # uniformly supported (notably on the bundled Fireworks beta), so we
            # keep `json_object` here to avoid 400s; the schema, when given, still
            # shapes the prompt and the parser stays the safety net (§23.2).
    ...
    def complete(
        ...
        payload = self._get_payload(messages, stream=False, temperature=temperature, top_p=top_p, response_format=response_format, stop_sequences=stop_sequences, max_tokens=max_tokens, response_schema=response_schema)
    ...
    def stream_tokens(
        ...
        payload = self._get_payload(messages, stream=True, temperature=temperature, top_p=top_p, response_format=response_format, stop_sequences=stop_sequences, max_tokens=max_tokens, response_schema=response_schema)
