...
def add_populate_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help="What to generate (repeatable: -t entities -t lore).",
    )
    parser.add_argument(
        "--text",
        help="Free-form instruction (custom mode) — otherwise inferred from existing lore.",
    )
...
def run_populate(args: argparse.Namespace) -> int:
    ...
    for target in args.target:
        try:
            ...
            print(f"LLM error ({target}): {exc}", file=sys.stderr)
            return 1
