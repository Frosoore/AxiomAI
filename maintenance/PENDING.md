...
| TICKET-069| **Validation Windows sur machine réelle** : 🔄 **gros lot fait le 2026-06-14** (crash WinError 32 résolu, classe de bugs connexion-non-fermée corrigée, suite 753✅ sous Windows — cf. `TICKET-062-windows-support/CHANGELOG.md`). Reste : audio `.ogg`, images locales, run.bat de bout en bout | ouvert — allégé |
| TICKET-070| **torch ne charge pas sous Windows** (`OSError WinError 126` sur `torch_python.dll`) : **VC++ Redistributable x64 manquant** sur la machine. App dégradée gracieusement (mémoire sémantique no-op + warning), mais à résoudre pour la feature → **installer vc_redist.x64.exe** ; envisager un check au démarrage (`run.bat`/`startup_check`) qui détecte l'échec d'import torch et pointe le redist | ouvert — environnement + robustesse onboarding |
| TICKET-071| App-layer : `with sqlite3.connect(...) as conn:` non-fermant restant dans `workers/db_worker.py` (×6) et `workers/import_export_worker.py` (×2). **Non bloquant** (pas de swap adjacent → fermé par refcount), mais même idiome latent que la classe corrigée au moteur. À convertir en `closing()` par hygiène | ouvert — quick win, faible risque |
