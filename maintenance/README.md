...
| `B2-pilier2-universe-as-code` | 🟡 en cours | Phase B §7 — Pilier 2 : Universe-as-Code. Univers = arbo texte TOML/MD versionnable, `.db` = cache compilé. MVP cœur : `axiom/compile.py` + `axiom/decompile.py` + CLI `compile`/`decompile` + `.axiom` v2 (compat v1) + tests round-trip. Différé : Creator Studio (Qt), hot reload `axiom dev`, séparation saves (§7.6), migration `Populate*`. |
