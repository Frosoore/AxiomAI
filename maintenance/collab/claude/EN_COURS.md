...
| `axiom/arbitrator.py` + `axiom/backends/{base,gemini,ollama,universal}.py` + `tests/{test_arbitrator,test_gemini_client,test_ollama_client,test_reasoning_models}.py` | pilier8-arbitrator-reliability **Lot 1 COMPLET** : (1) fix « plot armor » (plancher 0) ; (2) **bornes déterministes** (min/max + énums `allowed` dans `Stat_Definitions.parameters` JSON + `Location` ∈ lieux connus ; `_validate_change(…, stat_defs, known_locations)` → 3-uplet `(valid, reason, clamp_value)`) ; (3) **sortie structurée** — nouveau param `response_schema` sur `complete()`/`stream_tokens()`, **Gemini honore enfin `response_format="json"`** (`response_mime_type`+`response_json_schema`), Ollama `format`=schéma, UniversalClient reste `json_object`, Timekeeper passe en JSON+`_TIMEKEEPER_SCHEMA`. Fait, ~150 tests verts, en attente de commit | 2026-06-23 | ⚠ tour **narratif non concerné** par le JSON mode (prose streamée+fence inline) ; si tu touches un backend, **propage le param `response_schema`** (contrat abstrait) ; si tu touches `_validate_change`/`_load_stat_defs`/`_load_known_locations`, préserve le 3-uplet+clamp+signature. Suite = Lot 2 (détecteur divergence prose↔état, zéro LLM) |
